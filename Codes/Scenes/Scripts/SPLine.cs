...
namespace SPLINE
{
    ...
    class CubicSPLine
    {
        public static List<Vector3> GetFittingCubicSPLine(List<Vector3> linePoses, float gap = 0.01f)
        {
            /*
             * 功能：将传入的线条 三次样条插值，然后返回
             * 
             * 实现：
             * 
             * 参数： poses线条本体，offset为插值点间隔距离，
             *
             */

            //0. 计算这个线条的总长度，以划分成合适的段落
            //linePoses.Add(linePoses[0]);//首点存两遍

            float sumLength = 0.0f;
            for (int i = 0; i < linePoses.Count - 1; ++i)
            {
                sumLength += (linePoses[i+1]- linePoses[i]).magnitude;
            }

            int num = (int)(sumLength / gap) + 1;

            //1. 插值 初始化

            double[] x = new double[linePoses.Count];
            double[] _x = new double[linePoses.Count];
            double[] _z = new double[linePoses.Count];

            for (int i = 0; i < linePoses.Count; ++i)
            {
                x[i] = i + 1;

                Vector3 pos = linePoses[i];
                _x[i] = pos.x; 
                _z[i] = pos.z;
            }

            double[] newX = new double[num + 1];
            float offset = (linePoses.Count - 1.0f) / num;
            ...
            SPLine spine_X = new SPLine();
            SPLine spine_Z = new SPLine();

            spine_X.Init(x, _x);
            spine_Z.Init(x, _z);

            List<Vector3> res = new List<Vector3>();
            float y = linePoses[0].y;
            for (int i = 0; i < newX.Length; i++)
            {
                float __x = (float)spine_X.Interpolate(newX[i]);
                float __z = (float)spine_Z.Interpolate(newX[i]);

                res.Add(new Vector3(__x, y, __z));
                //Debug.Log(new Vector3(__x, __y, __z));
            }

            Debug.Log("spline count : " + res.Count);
